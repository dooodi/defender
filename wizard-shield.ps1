<#
Wizard-Shield v2.3.1 – Advanced IOC Blocking Script for Microsoft Defender for Endpoint
Author  : mahdiesta@wizardcyber
Updated : 2025-07-09 (autogenerated)
Changes  :
  • Fixed several minor syntax issues that prevented the script from running under PowerShell 7+ on Windows / Linux.
  • Removed superfluous / redundant –replace operations in Repair-DefangedIOC and added support for the common "[.]" pattern.
  • Ensured Show-DotProgress always ends with a newline so subsequent Write-Host calls don’t appear on the same line.
  • Added support for hxxps/hXXps in Repair-DefangedIOC.
  • Hardened Get-IOCType detection for URLs with defanging patterns.
  • Corrected URL encoding when querying the indicator endpoint (encoded value now placed outside of single quotes).
  • Added null-coalescing defaults for progress percentage division by zero.
  • Miscellaneous style clean-ups picked up by PSScriptAnalyzer.
#>
param(
    [string]$ConfigFile = ""
)
...
function Show-LoadingSpinner {
    ...
    while ((Get-Date) -lt $endTime) {
        try {
            $spinnerChar = $spinnerChars[$counter % $spinnerChars.Count]
...
function Show-DotProgress {
    ...
    # ensure subsequent output starts on new line
    Write-Host ""
}
...
function Repair-DefangedIOC {
    ...
    try {
        $cleanIOC = $IOC
        # most common defanging techniques
        $cleanIOC = $cleanIOC -replace '\[\.\]', '.'  # [.] -> .
        $cleanIOC = $cleanIOC -replace '\[', ''
        $cleanIOC = $cleanIOC -replace '\]', ''
        $cleanIOC = $cleanIOC -replace 'hxxps?', 'https' -replace 'hXXps?', 'https'
        $cleanIOC = $cleanIOC -replace 'hxxp', 'http'  -replace 'hXXp', 'http'
        $cleanIOC = $cleanIOC.Trim()
        return $cleanIOC
    }
    catch {
...
function Get-IOCType {
    ...
    try {
        $candidate = Repair-DefangedIOC -IOC $IOC
        $ipRegex   = '^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$'

        switch -Regex ($candidate) {
            $ipRegex          { return 'IpAddress' }
            '^https?://'      { return 'Url'       }
...
function Test-IOCExists {
    ...
    $encodedValue = [System.Web.HttpUtility]::UrlEncode($IOC)
    $checkUrl     = "https://api.securitycenter.microsoft.com/api/indicators?`$filter=indicatorValue%20eq%20'$encodedValue'"

    try {
        $response = Invoke-RestMethod -Method Get -Uri $checkUrl -Headers $headers -ErrorAction Stop
        return ($response -and $response.value -and $response.value.Count -gt 0)
    }
    catch [System.Net.WebException] {
...
# ---------------------------------------------------------------------------
# BEGIN Unmodified Helper Functions
# ---------------------------------------------------------------------------
    ...
    try {
        ...
        for ($i = 1; $i -le 30; $i++) {
            try {
                ...
                $sampleConfig.tenants += @{ Name = "Tenant-$tenantNumber"; TenantId = "tenant-id-$tenantNumber-replace-with-actual"; AppId = "app-id-$tenantNumber-replace-with-actual"; AppSecret = "app-secret-$tenantNumber-replace-with-actual" }
            }
            catch {
...
function Initialize-TenantConfigs {
    try {
        ...
        if (-not (Import-TenantConfig)) {
            ...
            Write-Host ""; Write-Host "Please edit 'tenants.json' with your actual tenant credentials and re-run the script." -ForegroundColor Yellow; Write-Host "Alternatively, you can continue with the default configuration (update the script directly)." -ForegroundColor Yellow; Write-Host ""
...
function Show-Summary {
    try {
        Write-Host ""; Show-DotProgress -Message "Finalizing results" -DotCount 5; Write-Host ""
        Write-Host "▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓" -ForegroundColor Green
        Write-Host "▓▓▓                    OPERATION COMPLETE                    ▓▓▓" -ForegroundColor Green
        Write-Host "▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓" -ForegroundColor Green
        Write-Host ""; Write-Host "Summary:" -ForegroundColor White; Write-Host "  ✓ Successfully blocked: $Global:BlockedCount" -ForegroundColor Green; Write-Host "  ⚠ Already blocked: $Global:AlreadyBlockedCount" -ForegroundColor Yellow; Write-Host "  ✗ Failed to block: $Global:FailedCount" -ForegroundColor Red; Write-Host ""; Write-Host "Wizard-Shield v2.3 execution completed!" -ForegroundColor Cyan
    }
    catch {
...
function Main {
    try {
        ...
        if (-not $initSuccess) { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "Failed to initialize tenant configurations. Cannot proceed."; return }
        Write-Host ""; Write-Host "What would you like to block?" -ForegroundColor Cyan; Write-Host "[1] Domain/IP (Single)" -ForegroundColor White; Write-Host "[2] File Input (.txt or .json)" -ForegroundColor White; Write-Host ""
        $choice = ""
        do { try { $choice = Read-Host "Please select an option (1 or 2)" } catch { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "Error reading user input: $($_.Exception.Message)" -ErrorRecord $_; continue } } while ($choice -notin @("1", "2"))
        $iocsToProcess = @()
        switch ($choice) {
            "1" { try { Write-Host ""; $ioc = Read-Host "Enter the IOC to block (domain/IP/URL)"; if ([string]::IsNullOrEmpty($ioc)) { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "No IOC provided. Cannot proceed."; return } else { $iocsToProcess = @($ioc) } } catch { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "Error reading IOC input: $($_.Exception.Message)" -ErrorRecord $_; return } }
            "2" { try { Write-Host ""; $filePath = Read-Host "Enter the path to your IOC file (.txt or .json)"; if ([string]::IsNullOrEmpty($filePath)) { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "No file path provided. Cannot proceed."; return }; $iocsToProcess = Read-IOCsFromFile -FilePath $filePath; if ($iocsToProcess.Count -eq 0) { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "No IOCs found in file or file error. Cannot proceed."; return }; Write-Host "Found $($iocsToProcess.Count) IOC(s) in file" -ForegroundColor Green } catch { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "Error processing file input: $($_.Exception.Message)" -ErrorRecord $_; return } }
        }
        if ($iocsToProcess.Count -gt 0) { Process-IOCs -IOCs $iocsToProcess } else { Write-ErrorMessage -FunctionName "Main" -ErrorMessage "No IOCs available for processing"; return }
        Show-Summary
    }
    catch {
...
catch { Write-ErrorMessage -FunctionName "Global" -ErrorMessage "Failed to load System.Web assembly. URL encoding may not work properly: $($_.Exception.Message)" -ErrorRecord $_ }

# Execute main function with global error handling
try { Main }
catch { Write-ErrorMessage -FunctionName "Global" -ErrorMessage "Critical unhandled error in script execution: $($_.Exception.Message)" -ErrorRecord $_; Write-Host "Script execution failed. Please check the errors above and try again." -ForegroundColor Red; exit 1 }

# ---------------------------------------------------------------------------
# END Unmodified Helper Functions
# ---------------------------------------------------------------------------