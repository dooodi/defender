...
# Function to escape OData filter values
function Escape-ODataValue {
    ...
    try {
        ...
        Write-ErrorMessage -FunctionName "Escape-ODataValue" -ErrorMessage "Failed to escape OData value '$Value': $($_.Exception.Message)" -ErrorRecord $_
        return $Value
    }
}
...
function Get-IOCType {
    ...
    try {
        ...
        # Check if it's a file hash (SHA256, SHA1, MD5)
        elseif ($IOC -match '^[a-fA-F0-9]{64}$') {
            return "FileSha256"
        }
        elseif ($IOC -match '^[a-fA-F0-9]{40}$') {
            return "FileSha1"
        }
        elseif ($IOC -match '^[a-fA-F0-9]{32}$') {
            return "FileMd5"
        }
...
function Test-IOCExists {
    ...
    try {
        ...
        # Properly escape the IOC value for OData filter
        $escapedValue = Escape-ODataValue -Value $IOC
        $checkUrl = "https://api.securitycenter.microsoft.com/api/indicators?`$filter=indicatorValue eq '$escapedValue'"
        ...
        if ($statusCode -eq 400) {
            Write-ErrorMessage -FunctionName "Test-IOCExists" -ErrorMessage "Bad request when checking IOC existence for '$IOC'. Possible invalid filter syntax." -ErrorRecord $_
        } else {
...
function Block-IOC {
    ...
    try {
        ...
        $statusCode = $_.Exception.Response.StatusCode
        if ($statusCode -eq 400) {
            Write-ErrorMessage -FunctionName "Block-IOC" -ErrorMessage "Bad request when blocking IOC '$IOC' in tenant '$TenantName'. Check IOC format and type." -ErrorRecord $_
        } elseif ($statusCode -eq 409) {
            Write-ErrorMessage -FunctionName "Block-IOC" -ErrorMessage "Conflict when blocking IOC '$IOC' in tenant '$TenantName'. IOC may already exist." -ErrorRecord $_
        } else {
        ...
        switch ($extension) {
            ".txt" {
                try {
                    ...
                    $filteredContent = $content | Where-Object { $_ -and $_.Trim() -ne "" -and -not $_.Trim().StartsWith("#") }
...
function Initialize-TenantConfigs {
    try {
        ...
        Write-Host "Ready to process IOCs across all configured tenants!" -ForegroundColor Cyan
        return $true
    }
    catch {
...
function Main {
    try {
        ...
        Write-Host "[1] Domain/IP/Hash (Single)" -ForegroundColor White
        ...
        switch ($choice) {
            "1" {
                try {
                    Write-Host ""
                    $ioc = Read-Host "Enter the IOC to block (domain/IP/URL/hash)"
                    if ([string]::IsNullOrEmpty($ioc)) {
